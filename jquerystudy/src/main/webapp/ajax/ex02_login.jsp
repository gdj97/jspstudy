...
<c:if test="${!empty rs.rows}"><%--db의 결과가 존재: id,pass가 맞는 레코드 존재 --%>
	<h1>반갑습니다 ${rs.rows[0].name}님</h1><%-- 클라이언트 전송 메세지 --%>
</c:if>
<c:if test="${empty rs.rows}"><%--db의 결과가 없음: id,pass가 맞는 레코드 없음 --%>
	<h1><font color="red">아이디 또는 비밀번호가 틀립니다.</font></h1> <%-- 클라이언트 전송 메세지 --%>
</c:if>
