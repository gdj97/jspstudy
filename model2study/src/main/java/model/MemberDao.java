...
public class MemberDao {
	...
	public boolean update(Member mem) {
		SqlSession session = DBConnection.getConnection();
		try {
			if(session.getMapper(cls).update(mem) > 0) return true;
			else return false;
		} catch(Exception e) {
			e.printStackTrace();
		} finally {
	...
	public String pwSearch(String id, String email, String tel) {
		SqlSession session = DBConnection.getConnection();
		try {
			return session.getMapper(cls).pwSearch(id,email,tel); //1번째 컬럼의 값
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			DBConnection.close(session);
	...
	public boolean updatePassword(String id, String pass) {
		SqlSession session = DBConnection.getConnection();
		try {
			return session.getMapper(cls).updatePassword(id,pass) > 0;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			DBConnection.close(session);
