...
public class DispatcherServlet extends HttpServlet {
	...
	private Properties pr = new Properties(); //url.properties 파일의 내용을 Map 객체로 저장
    ...
    /*
     *  url.properties 파일을 읽어서, pr객체에 key=value 쌍인 객체로 저장 
     */
    @Override
    public void init(ServletConfig config) throws ServletException {
    	...
    	try {
    		//config.getServletContext().getRealPath("/") : 웹어플리케이션 폴더
    		//f : url.properties 파일을 읽기 위한 입력스트림
    		f = new FileInputStream(config.getServletContext().getRealPath("/") + "WEB-INF/"+props);
    		pr.load(f); //Properties : Hashtable클래스의 하위클래스. <String,String> Map 객체
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(view.isRedirect()) { //true 인경우 redirect 함
			response.sendRedirect(view.getView());
		} else { //false 인 경우 forward 시킴
