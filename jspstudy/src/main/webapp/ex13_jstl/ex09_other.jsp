<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%-- /src/main/webapp/ex13_jstl/ex09_other.jsp --%>
...
급여:${param.salary }<br>
담당업무:<c:forEach items="${paramValues.job}" var="j">${j }&emsp;</c:forEach><br>
<hr>
<h3>입사일을 yyyy년 MM월 dd일 E요일의 형태로 출력하고, 급여,연봉을 세자리마다 ,로 넣어 출력하기. <br>연봉: 급여*12</h3>
이름:${param.name}<br>
<c:catch var="dateexception">
<fmt:parseDate value="${param.hiredate}" pattern="yyyy-MM-dd" var="hiredate" />
</c:catch>
입사일:
<c:if test="${dateexception == null}"><%-- 정상 처리 --%>
   <fmt:formatDate value="${hiredate}" pattern="yyyy년 MM월 dd일 E요일" /><br>
</c:if>   
<c:if test="${dateexception != null}"><%-- 예외 처리 --%>
