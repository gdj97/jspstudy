<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%-- /src/main/webapp/ex13_jstl/ex05_fmt_number.jsp --%>
...
<h3>숫자 관련 형식 지정</h3>
<%--
   type="currency" : 현재 지역의 화폐단위 출력. 브라우저에서 전달되는 값 설정.
   currencySymbol="￦" : 지역의 화폐표시 문자 지정
   
   type="percent" : value 의 값을 퍼센트단위로 출력
   
   pattern="###,###.00" : 사용자가 숫자의 형식을 지정
 --%>
<fmt:formatNumber value="500000" type="currency" currencySymbol="￦"/>원<br> <%-- ㄹ한자 --%>
...
년월일시분초-GMT:<fmt:formatDate value="${today}" type="both" timeStyle="full" dateStyle="full" timeZone="GMT"/><br>
<h3>직접 형식을 지정하여 출력하기</h3>
형식지정 : <fmt:formatDate value="${today}" pattern="yyyy년 MM월 dd일 HH:mm:ss a E"/>
<p>pattern 속성은 SimpleDateFormat에서 사용하는 형식화문자와 같다</p>
<%--
    ~~Format 클래스
    	format() : 객체 => 형식화된 문자열 변경
    		<fmt:formatNumber ... > : 숫자 => 형식화된 문자열 변경
    		<fmt:formatDate ...>    : Date 객체 => 형식화된 문자열로 변경
    	parse() : 형식화된 문자열 => 원래의 객체	
    		<fmt:parseNumber ... > : 숫자 <= 형식화된 문자열 변경
    		<fmt:parseDate ...>    : Date 객체 <= 형식화된 문자열로 변경
 --%>
</body>
</html>