...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%-- /src/main/webapp/member/updateForm.jsp
  1. 로그인 상태 검증
...
<% } else if (!id.equals(login) && !login.equals("admin")) {  //로그인 상태. 본인정보검증%>
<script type="text/javascript">
...
<tr><td colspan="3"><button>회원수정</button><%-- submit버튼 --%>
<% if(id.equals(login)) { //본인 정보인 경우만 비밀번호 수정이 가능함%>
<button type="button" onclick="win_passchg()">비밀번호수정</button>
<% }  %>
</td></tr>
</table></form>
<%--
   회원등록시 검증, 회원수정 검증이 동일함
 --%>
